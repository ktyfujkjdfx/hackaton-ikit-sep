...
> Банки показывают, что уже произошло с деньгами. Мы — для тех, кто распоряжается деньгами впервые. Отличаем постоянные деньги от разовых, честно показываем, где факт, а где оценка, отвечаем не «хватит ли», а «когда можно купить, не выходя за границы бюджета», и на каждом шаге объясняем, как посчитали.

**Киллер‑фичи (должны работать и быть заметны):**
1. **«Когда можно купить»** — самая ранняя дата, с которой покупка не выводит за границы бюджета, плюс дата исчерпания средств и размер превышения.
...
3. **Как остаться в границах бюджета** — 3 варианта: перенести покупку / сократить расходы на N ₽ в день / найти N ₽ к дате. Метка «легко» или «сложно». Вакансии — только карточки с пометкой «Пример».
...
5. **Учим, а не советуем** — карточка «Как устроены накопления и инвестиции?» одинаковая для всех, без сумм пользователя, не показывается при превышении бюджета. На «куда вложить» AI вежливо отказывает и показывает эту карточку.
...
| Деплой (без Docker) | **Локальный запуск + VDS за реверс-прокси:** бэкенд на 8000, статика фронта рядом, `/api` проксируется на бэкенд. `render.yaml` оставлен как запасной вариант | не зависим от чужого бесплатного тарифа |
...
```
Frontend (`frontend/.env.production`): `VITE_API_URL` пустой — собранный фронт ходит в API по тому же адресу, с которого отдан. Локально работает прокси Vite. Если API на отдельном домене — задать `VITE_API_URL` при сборке и добавить домен в `CORS_ORIGINS`.
...
{ "today": "2026-09-27", "horizon_days": 30,
  ...
  "assumptions": ["Обычные траты — 300 ₽ в день, оценка по истории за 2 месяца",   // «из анкеты», если history нет
...
{ "passed": 13, "total": 13,
  "items": [{ "id": 1, "title": "Аня без покупок",
              "expected": "минимум 600 ₽, 9 октября; бюджета хватает; до стипендии 13 дней",
              "got": "600 ₽, 9 октября; бюджета хватает; Стипендия, 13 дней", "ok": true }] }
...
{ "intent": "purchase_check" | "forecast" | "explain" | "deficit_plan" | "categories" |
  ...
  "headline": "Если купить сейчас, вы выйдете за границы бюджета",
  "tone": "good" | "bad" | "neutral",
  "facts": [{ "label": "Дата, исчерпание средств", "value": "5 октября", "tone": null },   // tone: "good" | "bad" | null
            { "label": "Максимальное превышение бюджета", "value": "2 400 ₽", "tone": "bad" }],
  "text": "Если подождать до 15 октября, покупка не выведет за границы бюджета. Решение за вами.",
...
1. Старт → «Демо: студентка Аня» → «До основного поступления средств **13 дней**. Денег достаточно до 9 октября, останется **600 ₽**».
2. В верхнем блоке: «Наушники», 3000, сегодня → «Когда можно купить?» → карточка: **«Без превышения бюджета можно купить с 15 октября»**, превышение с 5 октября до 2 400 ₽, цель сдвинется на 50 дней, три варианта выхода.
3. Жюри называет сумму (например, 1 000) → пересчёт вживую: превышение бюджета с 8 октября, безопасно с 10 октября.
4. «Как посчитали?» → меняем 300 → 250 ₽ в день → график и ответ меняются на глазах.
5. Блок «Постоянное и разовое» → Даня: пунктир и «если подработки не будет — превышение бюджета с 9 октября».
