# Роль D — Данные, деплой, исследование

> Команда «ФинКом» · 4 человека · 26–27 сентября 2026 · стоп‑код 11:00 (Красноярск).
...
> Твоя работа приносит баллы за «Анализ» и «Документацию» и держит сайт живым. Кода почти нет; репозиторий создаёт тимлид (C), финальную проверку и демо ведёт тоже он. Ты помогаешь.

## D.0 Твоя зона за 30 секунд

- Файлы: `README.md`, `render.yaml`, `.github/workflows/ci.yml`, `backend/app/data/personas.json`, `backend/app/data/knowledge.json`, `docs/pitch/`.
- Результат:
  1. данные персон и мини‑база терминов — к 18:30;
  2. два сервиса на Render (без Docker) и CI — к 19:00;
  3. 40 тестовых фраз для модели B — к 19:30;
  4. исследование с источниками и мини‑опрос;
  5. README;
  6. слайды (тимлид их проверяет).

## D.1 Час 0 (16:40–17:20)
Прочитай этот файл. Пока тимлид создаёт репо, заведи аккаунт на Render (вход через GitHub) и создай форму опроса (D.5), чтобы в 19:00 её только запустить.

## D.2 Данные (17:20–18:30)

1. **`backend/app/data/personas.json`** — ровно JSON из раздела 8 (скопируй блок). Пуш **до 18:00**, напиши A.
2. **`backend/app/data/knowledge.json`** — 8 терминов:
   ```json
   [{ "key": "подушк", "title": "Финансовая подушка",
      "text": "Запас денег на случай, если доход пропал или случилась непредвиденная трата…",
      "source_title": "fincult.info — сайт Банка России", "source_url": "https://fincult.info/…" }]
   ```
   Термины: финансовая подушка, кассовый разрыв, рассрочка, кредитный лимит, льготный период по кредитной карте, «сплит» (оплата частями), вклад и накопительный счёт (объяснение, **без советов**), инфляция. Тексты — 2–3 простых предложения. **Каждое определение сверь с живой страницей fincult.info и вставь её точный URL** (ссылки не выдумывай). Пуш до 18:30, напиши B.
3. Найди ссылку на обучающий курс Т‑Банка про накопления и инвестиции (например, в Т‑Образовании или Т—Ж), открой её и отправь тимлиду для `LearnModal`.

## D.3 Деплой без Docker (18:00–19:00)
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: DEMO_TODAY
        value: 2026-09-27
      - key: CORS_ORIGINS
        value: https://dotyanu.onrender.com,http://localhost:5173
      - key: YANDEX_API_KEY
        sync: false
  ...
  - type: web
    ...
    envVars:
      - key: VITE_API_URL
        value: https://dotyanu-api.onrender.com
```
- Render → New → **Blueprint** → репозиторий → применить. Если blueprint не заведётся, создай два сервиса вручную в интерфейсе с теми же настройками: **Web Service** (Python, Root `backend`) и **Static Site** (Root `frontend`, Publish `dist`).
- Если имена `dotyanu` / `dotyanu-api` заняты, Render даст другие адреса. Тогда поправь `CORS_ORIGINS` и `VITE_API_URL` и сообщи адреса в чат.
- Ключи API (если B решит их использовать) вводишь сам в интерфейсе Render. B передаёт их лично, не в общий чат.
- Автодеплой из `main` включён. **К 19:00: оба адреса открываются**, `https://dotyanu-api.onrender.com/api/health` отвечает. Адреса — в чат и в README.
- **Важно:** бесплатный backend засыпает после ~15 минут простоя, и первый запрос идёт до минуты. Перед защитой открой `/api/health` за 3–5 минут и держи вкладку открытой.

**`.github/workflows/ci.yml`:** на push и PR два job: `backend` (Python 3.11, `pip install -r backend/requirements.txt`, `cd backend && pytest -q`) и `frontend` (Node 20, `cd frontend && npm ci && npm run build`). Напиши тимлиду, когда CI заработает: он включит защиту `main`.

## D.4 40 тестовых фраз для модели B (19:00–19:30)
B обучает свою нейросеть понимать вопросы. Чтобы честно её проверить, нужны фразы, которых B не видел. **Не смотри датасет B.** Напиши 40 фраз так, как спросил бы живой студент (сленг, опечатки, без знаков препинания), по 3–4 на каждое намерение из раздела 9. Формат — `training/data/intents_holdout_team.jsonl`, одна строка на фразу: `{"text": "скок у меня останется до стипухи", "label": "forecast", "author": "D"}`. Отдай B файлом или PR.

## D.5 Исследование и опрос (19:30–00:30) — главный источник баллов «Анализа»

Всё собирай в `docs/pitch/research.md`: **у каждой цифры — источник и год**. ИИ‑поиском пользоваться можно, но **каждую цифру открой и проверь на первоисточнике** — отдельный критерий оценивает достоверность данных.

| Критерий (баллы) | Что найти | Где искать |
|---|---|---|
| Рынок и вопрос исследования (4) | Вопрос: «Нужен ли студентам первого года инструмент, который до покупки показывает, хватит ли денег до следующего поступления?» Границы: Россия → Красноярск и СФУ, очные студенты 18–22 лет, 2026 год | — |
| Ёмкость и динамика (6) | число студентов вузов в РФ и динамика за 3–5 лет; число первокурсников; доля живущих в общежитиях; студенты СФУ и их общежития; размер стипендий; сколько молодёжи пользуется банковскими приложениями | Росстат, Минобрнауки, сайт СФУ, НАФИ, Банк России |
| ЦА и проблема (7) | цифры о нехватке денег у студентов (например, опрос МК 2024: 57% — стипендии не хватает; перепроверь), **наш опрос** и 3 коротких интервью с однокурсниками | опрос в чатах ИКИТ/СФУ |
| Конкуренты (5) | таблица: приложения банков (аналитика трат), Дзен‑мани, CoinKeeper, YNAB, ассистенты Т‑Банка (Финассистент, «Тая» — бета 09.09.2026), GitHub‑проекты Finch и FIN‑AI, победители краснодарского хакатона 2026 («Заначка», «Эклерк», «Финеело») | github.com/varshithaasubramanian/Finance-Agent, github.com/Dhruv5911/FIN-AI, bosfera.ru (новость о «Тае»), kommersant.ru/doc/8832032, сайты приложений |
| Рыночные факторы (4) | Кодекс этики ИИ Банка России (2025), 152‑ФЗ о персональных данных, тренд ИИ‑ассистентов в банках, стоимость жизни студентов | cbr.ru, новости |
| Качество данных (3) | у каждой цифры источник и год; наши оценки явно помечены | — |
| Обоснованность идеи (6) | цепочка: боль (опрос) → чего нет у конкурентов (дата «когда можно», постоянное и разовое, прозрачность) → наше решение → как проверим дальше (пилот, метрики) | — |

**Мини‑опрос** (Яндекс Формы или Google Forms). Запустить **в 19:00** в чатах курса, цель — 20–40 ответов к 02:00:
...
Итоги — в `docs/pitch/survey.md` и один график для слайда.

## D.6 README (черновик к 01:00, финал к 10:00) — 5 баллов
Разделы README:
- что это и для кого (3 строки);
- **ссылки на сайт и API** и скриншот;
- как пользоваться (путь демо, раздел 11);
- как запустить локально (PowerShell и bash);
- пример входных данных (кусок `personas.json`);
- API (ссылка на `/docs` и таблица эндпоинтов);
- архитектура (схема: браузер → API → движок / модель);
- как устроен AI и как обучить модель (текст и 3 команды от B, ссылка на `docs/ai/model_card.md`);
- проверки (`docs/checks.md` и страница «Как мы проверяли»);
- **известные ограничения**: прогноз на 30 дней, средние траты — оценка, демо‑дата фиксирована, модель обучена на синтетике, данные не сохраняются;
- команда и роли.

## D.7 Слайды (черновик к 00:30, версия 1 к 03:00, финал к 09:30; тимлид проверяет)
10–11 слайдов, каждый закрывает критерий:
1. Название, команда, одна фраза о продукте.
2. Проблема и сегмент: цифры и цитаты из опроса — *ЦА и проблема*.
3. Рынок: границы, ёмкость, динамика — *Рынок, Ёмкость*.
4. Конкуренты и чего им не хватает — *Конкуренты*.
5. Решение: главный вопрос и 4 фишки — *Обоснованность*.
6. Как работает: «данные → наша модель понимает → движок считает → ответ с метками» — *Ясность*.
7. Своя модель: данные, метрики на фразах команды, матрица ошибок (от B).
8. Живое демо (запасной вариант — скриншоты).
9. Как мы проверяли: 13/13; риски (ошибки ввода, подработка, забывчивость) и наши решения — *Корректность*.
10. Ограничения и безопасность: синтетика, без инвестсоветов и операций, кодекс ЦБ — *Рыночные факторы*.
11. Ценность для Т‑Банка и что дальше (пилот на 50 студентах СФУ; метрики: возвращаемость за 7 дней, доля покупок, проверенных до оплаты). QR‑коды на сайт и репозиторий.

## D.8 Таймлайн коротко

| Время | Что |
|---|---|
| 16:40–17:20 | прочитать план; аккаунт Render; форма опроса |
| 17:20–18:30 | personas.json (до 18:00), knowledge.json (до 18:30), ссылка на курс Т‑Банка |
| 18:00–19:00 | render.yaml, оба сервиса, CI |
| 19:00–19:30 | запуск опроса; 40 фраз для B |
| 19:30–00:30 | research.md, конкуренты, черновик слайдов, черновик README |
| 00:30–03:00 | слайды v1, итоги опроса, помощь тимлиду с репетицией в 01:00 |
| 03:00–07:00 | сон |
| 07:00–08:30 | финал слайдов, скриншоты, картинка модели от B |
| 08:30–10:00 | помогаешь тимлиду с финальной проверкой; README финал |
| 10:00–10:40 | проверка README и ссылок, тег `v1.0` вместе с тимлидом |
| 10:40–11:00 | репетиция: ты говоришь, тимлид показывает |

## D.9 Стартовый промпт для Claude Code

```
Репозиторий hackaton-ikit-sep. Я — роль D: данные, деплой, исследование.
Прочитай CLAUDE.md, docs/CONTRACT.md (разделы 2, 3, 8, 9) и docs/plans/ROLE_D_data_devops_pitch.md.
Пиши только в README.md, render.yaml, .github/workflows/ci.yml, backend/app/data/, docs/pitch/.
Docker не используем.
Задача сейчас:
1) backend/app/data/personas.json — ровно JSON из раздела 8 CONTRACT.md; проверь, что он валидный.
2) render.yaml и .github/workflows/ci.yml по разделу D.3 моего плана.
Покажи diff.
```

## Чек‑лист D перед стоп‑кодом
- [ ] Оба сайта работают, адреса есть в README и на последнем слайде
- [ ] `research.md`: у каждой цифры источник и год
- [ ] Опрос: ≥ 20 ответов, график на слайде
- [ ] 40 фраз отданы B
- [ ] Слайды готовы, тимлид их проверил
- [ ] README по пунктам D.6
