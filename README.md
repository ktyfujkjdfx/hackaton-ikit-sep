...
> Бесплатный сервер на Render засыпает после 15 минут простоя. Первый запрос после сна ответил за 27 секунд (замер 26.09.2026), дальше ответы идут за 0,5–0,6 с.
...
- **Бесплатный хостинг:** сервер засыпает после 15 минут без запросов. Замер 26.09.2026: после 18 минут простоя первый ответ `/api/health` пришёл через **27 секунд**, следующий `/api/dashboard` — через 0,6 с, тёплые запросы — 0,5–0,6 с. Перед демо открываем сайт заранее.
