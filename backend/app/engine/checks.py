...
STATE_RU = {"ok": "хватает", "tight": "бюджета хватает", "deficit": "превышение бюджета",
            "depends": "зависит от подработки",
...
def check_2():
    ...
    got = (f"превышение бюджета с {fd(r.after.first_negative_date)}, до {format_rub(r.after.max_deficit)}; "
...
def check_4():
    ...
    st = stats(sit, series(sit))
    h = headline(sit)
    got = f"превышение бюджета с {fd(st.first_negative_date)}; {STATE_RU[h.state]}"
...
def check_7():
    ...
    return f"превышение бюджета с {fd(r.after.first_negative_date)}", r.after.first_negative_date == TODAY
...
def check_8():
    ...
    st = stats(sit, series(sit))
    got = f"превышение бюджета {format_rub(st.first_negative_amount)} с {fd(st.first_negative_date)}"
...
def check_10():
    ...
    got = (f"с подработкой: мин {format_rub(base.min)}, {fd(base.min_date)}; {STATE_RU[h.state]}; "
           f"без: превышение бюджета с {fd(pess.first_negative_date)}, до {format_rub(pess.max_deficit)}")
...
CHECKS: list[Check] = [
    Check(1, "Аня без покупок",
          "минимум 600 ₽, 9 октября; бюджета хватает; до стипендии 13 дней", check_1),
    Check(2, "Аня покупает наушники за 3 000 ₽ сегодня",
          "превышение бюджета с 5 октября, до 2 400 ₽; без превышения — с 15 октября; "
          "сократить расходы на 185 ₽/день до 14 октября, 18 дней", check_2),
    Check(3, "Жюри: покупка за 1 000 ₽ сегодня",
          "превышение бюджета с 8 октября, до 400 ₽; без превышения — с 10 октября; "
          "сократить расходы на 31 ₽/день",
          check_3),
    Check(4, "Пустой остаток: 0 ₽, траты 300 ₽/день, поступлений нет",
          "превышение бюджета с сегодняшнего дня, нет даты дохода", check_4),
    ...
    Check(7, "Покупка ровно на весь остаток Ани (6 900 ₽)", "превышение бюджета с сегодняшнего дня", check_7),
    Check(8, "Платёж и поступление в один день: 1 000 ₽, 100 ₽/день, 30 сентября +2 000 и −3 000",
          "превышение бюджета 400 ₽ с 30 сентября", check_8),
    ...
    Check(10, "Даня: подработка может не прийти",
          "с подработкой минимум 1 160 ₽ (19 октября); без неё превышение бюджета с 9 октября, до 2 840 ₽",
          check_10),
    Check(11, "Аня записала «такси 800» сегодня", "минимум −200 ₽; сократить расходы на 16 ₽/день",
          check_11),
